...
namespace
{
  ...
  int kdtree_size_st = 0, kdtree_delete_counter = 0, add_point_size = 0;
    ...
    : nav2_util::LifecycleNode("laser_mapping_lifecycle_node", "", options)
{
  ...
  this->declare_parameter<std::string>("tf.map_frame_id", "map");
  this->declare_parameter<std::string>("tf.odom_frame_id", odom_frame_id_);
  this->declare_parameter<std::string>("tf.sensor_frame_id", sensor_frame_id_);
  ...
  this->declare_parameter<bool>("enable_relocalization", true);
}
...
LaserMappingLifecycleNode::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  map_frame_id_ = this->get_parameter("tf.map_frame_id").as_string();
  odom_frame_id_ = this->get_parameter("tf.odom_frame_id").as_string();
  sensor_frame_id_ = this->get_parameter("tf.sensor_frame_id").as_string();
