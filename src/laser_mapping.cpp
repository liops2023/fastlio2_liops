 ...
 ********************************************************************************************/
 ...
 namespace
 {
   ...
   static void lasermap_fov_segment()
   {
     ...
     RCLCPP_INFO(rclcpp::get_logger("LIO"),
                 "[lasermap_fov_segment] Local map moved. kdtree_delete=%d, time=%.3f",
   ...
   static void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
   {
     ...
     p_pre->process(tmp, ptr); // preprocess
 
     lidar_buffer.push_back(ptr);
     time_buffer.push_back(cur_time);
     last_timestamp_lidar = cur_time;
     (void)preprocess_start_time; // 디버그용
 ...
 LaserMappingLifecycleNode::on_configure(const rclcpp_lifecycle::State &)
 {
   ...
   this->declare_parameter<double>("relocalization_score_threshold", 100.0);
   this->declare_parameter<int>("relocalization_retry_timeout_sec", 300);
   ...
   while (!reloc_stop_flag_) {
     ...
     if (elapsed_sec > reloc_timeout_sec_) {
       RCLCPP_WARN(get_logger(),
         "[reloc_retry_loop] Timeout(%d s) reached, stop attempts.",
         reloc_timeout_sec_);
       break;
     }
 
     // 2) 액션 전송
     RCLCPP_INFO(get_logger(), "[reloc_retry_loop] Sending re-localization request...");
     auto send_goal_options = rclcpp_action::Client<GetRelocPose>::SendGoalOptions();
     // 여기서는 "result_callback" 대신에 "동기적 Future wait"를 예시로 사용.
     auto future_goal_handle = reloc_action_client_->async_send_goal(goal_msg, send_goal_options);
 
     // 2-1) goal_handle 타임아웃(서버와 통신 안됨) 체크 (예: 5초)
     ...
     if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
       int score = result.result->score;  // 서버가 score를 함께 반환한다고 가정
     ...
     // 다음 시도 전 잠시 대기 (과도한 요청 방지)
     std::this_thread::sleep_for(std::chrono::seconds(1));
   }
 
   if (!success_done) {
     RCLCPP_WARN(get_logger(),
       "[reloc_retry_loop] Finished attempts without success or timed out.");
   } else {
